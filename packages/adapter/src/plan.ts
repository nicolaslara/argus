...
// The pure AST layer (the `AnyNode` structural view of an acorn node + the shape-readers and the
// label/condition/loop classifiers) lives in ./ast-helpers.ts (AV13) so this file stays the acorn
// wrap-parse + the PlanBuilder walk. `start`/`end` on AnyNode are byte offsets into WRAPPED source.
import {
  type AnyNode,
  isNode,
  labelTemplateFromNode,
  literalOrTemplateString,
  optStringLike,
  hasOptKey,
  objectPropValue,
  asArrayMapCall,
  findAgentCall,
  containsAgentCall,
  containsParallelCall,
  callContainsDslSomewhere,
  childExprs,
  classifyCondition,
