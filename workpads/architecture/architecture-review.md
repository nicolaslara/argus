...
| 2 | **`clientVersion` is dead-plumbed; no format-drift badge exists** | **MED** | No route ever sets `ctx.clientVersion`/`opts.clientVersion` (only type decls + `!== undefined` guards at `index.ts:259`, `live.ts:327`), so `RunModel.clientVersion` is always `undefined`. The "untested format" badge (`boundaries.md §9`) does not exist. The "format mismatch detected best-effort" claim is absent end-to-end. | Either wire `resolveClientVersion` into a route + render a drift badge when it `!== '2.1.161'`, or delete the dead plumbing and update `boundaries.md §9`. | M |
